...
> **vr200:** The `vr200` accelerator is supported on **Ubuntu 24.04 only**. Its
> profiles live under `profiles/os/ubuntu/24.04/`. With `service=eks`, vr200 keeps
> the reboot-requiring `[bootloader]` tuning (same as gb200). With `service=bcm`,
> vr200 uses a bootloader-free profile chain (`nvidia-vr200-noreboot-base`) so the
> tuning applies without a reboot.
